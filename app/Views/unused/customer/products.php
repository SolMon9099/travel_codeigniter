...
<?= $this->include('partials/body') ?>
                            ...
                            <div class="d-flex flex-row-reverse m-0 w-100">
                                <div class="row w-100" style="padding-right:20px;">
                                    ...
                                    <div class="dropdown col-md-2 mb-3" style="text-align: right; padding-top:10px;">
                                        <!-- Test -->
                                        <!--
                                        <a href="<?= base_url("CustomerController/send_whatsapp_message/75/0") ?>" class="btn btn-primary">TEST</a>
                                        -->
                                        <!-- Test -->
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-lg-start" style="min-width:350px; position:absolute; inset:0px auto auto 0px; margin:0px; transform:translate(-33px, 72px);" data-popper-placement="bottom-end">
                                            ...
                                            <?php
                                                if ($workshop_id != 0) {
                                            ?>
                                                    <span class="p-2" style="color: #092737;">Success! Workshop ID assigned.&nbsp;<a href="#" class="link-danger" style="color: #EE0263;" onclick="revokeWorkshop_Century();"><u>Revoke</u></a></span>
                                            <?php
                                                }
                                            ?>
                                            ...
                                            <h4 class="mb-0 font-size-18 m-3 mt-2">Enter Century ID (optional)</h4>
                                            
                                            <div class="input-group p-2">
                                                <input class="form-control" type="text" name="century_id" id="century_id" value="<?= $century_id == '' ? '' : $century_id ?>" placeholder="Century ID (optional)">
                                                <div class="input-group-append">
                                                    <button class="btn btn-primary input-group-text" onclick="validateCentury();"><i class="mdi mdi-arrow-right" style="font-size: 20px;"></i></button>
                                                </div>
                                            </div>
                                            ...
                                            <?php
                                                if ($century_id != '') {
                                            ?>
                                                    <span class="p-2" style="color: #092737;">Success! Century ID assigned.&nbsp;<a href="#" class="link-danger" style="color: #EE0263;" onclick="revokeWorkshop_Century();"><u>Revoke</u></a></span>
                                            <?php
                                                }
                                            ?>
                                        </ul>
    ...
    <script>
        ...
        var global_century_id = "<?= $century_id ?? "" ?>";
        ...
        function loadProduct() {
            $.post("<?= base_url("CustomerController/load_product") ?>", {
                ...
                for (var i = 0; i < p.length; i++) {
                    var price_html = "";
                    if (global_workshop_id != "0") {
                        if (parseFloat(p[i]['selling_price']) > parseFloat(p[i]['workshop_price'])) {
                            price_html = "<s>"+p[i]['selling_price']+"</s>&nbsp;"+p[i]['workshop_price'];
                        } else {
                            price_html = p[i]['workshop_price'];
                        }
                    } else if (global_century_id != "") {
                        if (parseFloat(p[i]['selling_price']) > parseFloat(p[i]['century_price'])) {
                            price_html = "<s>"+p[i]['selling_price']+"</s>&nbsp;"+p[i]['century_price'];
                        } else {
                            price_html = p[i]['century_price'];
                        }
                    } else {
                        price_html = p[i]['selling_price'];
                    }
                                ...
                                <form action="<?= base_url('customer/add_cart');?>" method="POST">
                                    <div class="card-body text-center p-2" style="display:flex; flex-direction:column; height:100%;">
                                        ...
                                        <div class="row" style="flex-grow:1;">
                                            ...
                                            <div class="col-12">
                                                <h6 style="color: #EE0263;">RM ` + price_html + `</h6>
                                            </div>
                                            ...
                                            <div class="col-12">
                                                <h6 style="color: #EE0263;">RM ` + 
                                                    (global_workshop_id != "0" 
                                                        ...
                                                        : global_century_id != ""
                                                            ? ("<s>"+p[i]['selling_price']+"</s>&nbsp;"+p[i]['century_price']) 
                                                            : p[i]['selling_price'])
                                                + `</h6>
                                            </div>
        ...
        function post_checkout_swal(type, message) {
            Swal.fire({
              ...
              backdrop: true,
                ...
                if (result.isConfirmed) {
                    var id = global_workshop_id != "0" ? global_workshop_id : global_century_id != "" ? global_century_id : "0";
                    
                    window.location.href = "<?= site_url('shop') ?>" + "/" + global_partner_id + "/" + id;
                }
            }).catch(swal.noop);
        }
        ...
        function displayProductDetail(product_id, workshop_id) {
            for (var i=0; i<products.length; i++) {
                if (products[i]['id'] == product_id) {
                    ...
                    } else if (global_century_id != "") { // century_id != ""
                        $('#modal-product-price').html("<s>" + products[i]['selling_price'] + "</s>&ensp;" + products[i]['century_price']);
                        $('#modal-product-warranty').text(products[i]['century_warranty_month'] + " Month(s)");
                    } else { // workshop_id == "0"
                    ...
                    for (var i=0; i<cart.length; i++) {
                        for (var j=0; j<products.length; j++) {
                            if (products[j]['id'] == cart[i][0]) {
                                ...
                                var price = workshop_id != "0" ? products[j]['workshop_price'] : global_century_id != "" ? products[j]['century_price'] : products[j]['selling_price'];
        ...
        function validateCentury() {
            var century_id = $('#century_id').val();
            
            if (century_id != "") {
                location.href = "<?= site_url('shop') ?>" + "/" + global_partner_id + "/" + century_id;
            }
        }
        
        function revokeWorkshop_Century() {
        ...
        function validateTradeIn() {
            ...
            for (var j=0; j<trade_in_types.length; j++) {
                ...
                var trade_in_type       = trade_in_types[j]['type'];
                ...
                if (isNaN(trade_in_quantity)) {
                    ...
                    if (trade_in_subtotal != 0.00 && cart_length > 0 ) {
                        var order_quantity = 0;
                        
                        for (var l=0; l<cart.length; l++) {
                            for (var m=0; m<products.length; m++) {
                                if (cart[l][0] == products[m]['id']) {
                                    for (var n=0; n<sizes.length; n++) {
                                        if (sizes[n]['product_size_id'] == products[m]['size_id']) {
                                            if (trade_in_type == sizes[n]['size_name']) {
                                                order_quantity += parseInt(cart[l][1]);
                                            }
                                        }
                                    }
                                }
                        ...
                        var elligible_quantity = 0;
                        if (order_quantity >= trade_in_quantity)
                            elligible_quantity = trade_in_quantity;
                        else if (trade_in_quantity > order_quantity)
                            elligible_quantity = order_quantity;
                        
                        var adjustment = (trade_in_types[j]['value_with_order'] - trade_in_types[j]['value_without_order']) * elligible_quantity;
            ...
            if (new_final_total <= 0.00) {
                ...
                document.getElementById("payment_method").setAttribute("disabled", "disabled");
            } else {
        ...
        function checkout() {
            ...
            Swal.fire({
              ...
              ` 
                ...
                <input name="century_id" id="century-id-hidden" value="`+ century_id +`" hidden>
                <input name="agent_id" id="agent-id-hidden" value="`+ agent_id +`" hidden>
                ...
                <input name="browser" id="browser-hidden" value="`+ browserName +`" hidden>
                ...
                <div class="col-md-12 mb-2" style="text-align: left;">
                    ...
                    <input class="form-control form-control-sm" type="text" name="customer_phone" id="customer_phone" >
                </div>
                <div class="col-md-12 mb-2" style="text-align: left;">
                    <label class="form-label font-size-14">Your Car License Plate <span class="text-danger">*</span></label>
                    <input class="form-control form-control-sm" type="text" name="license_plate" id="license_plate" >
                </div>
                <div class="col-md-12 mb-2" style="text-align: left;">
                    <label class="form-label font-size-14">Order Payment Method <span class="text-danger">*</span></label>
                    <select class="form-control form-control-sm" name="payment_method" id="payment_method">
                        <option value="0" hidden>Not Applicable</option>
                ...
                <div class="col-md-12 mb-2" style="text-align: left;">
                    ...
                    <!--1, Jln Bunga 3 Taman Oren 5-->
                    <textarea class="form-control form-control-sm" name="customer_address" id="customer_address" rows="3"></textarea>
                </div>
                <div class="col-md-12 mb-2" style="text-align: left;">
                    ...
                    <div class="input-group mb-2">
                        <input class="form-control form-control-sm" type="text" id="voucher-code" >
                        <div class="input-group-append">
              ...
              preConfirm: function () {
                if (!$('#customer_name').val() || !$('#customer_phone').val() || !$('#license_plate').val() || !$('#payment_method').val()) {
                            ...
                            if (status == "success") {
                                switch(condition) {
                                    case "1":
                                        ...
                                        break;
                                    case "2":
                                        if (payment_method == "0" || payment_method == "2") { // if payment method = not applicable / cash
                                            window.sessionStorage.removeItem('cart');
                                            swal_order_details(condition, order_id, trade_in_id, "Successfully placed an order. Partner will collect the remaining total of RM " + final_total + " from you. Thank you for purchase with Bateri Laju!");
                                        } else {
                                    ...
                                    case "3":
                                        if (parseFloat(final_total) < 0.00) {
                                            ...
                                            if (payment_method == "0" || payment_method == "2") { // if payment method = not applicable / cash
                                                window.sessionStorage.removeItem('cart');
                                                swal_order_details(condition, order_id, trade_in_id, "Successfully placed an order. A driver will come pickup the batteries and partner will collect the remaining total of RM " + final_total + " from you. Thank you for purchase with Bateri Laju!");
                                            } else {
                                                window.open(billplz_bill_link, "_self");
                                            }
                                        }
